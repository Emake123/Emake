...
-(void)getLeftListData{
    ...
    [[YHJsonRequest shared] getShoppingGoodCategoriesSeriesId:self.serriesCode  SuccessBlock:^(NSDictionary *successMessage) {
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row < self.dataArray.count-1) {
        ...
        if ([self.dataArray objectAtIndex:self.dataArray.count-1]) {
            UIImageView *ItemImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, WidthRate(120), HeightRate(120))];
            ItemImage.contentMode = UIViewContentModeScaleAspectFit;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.item == self.dataArray.count-1) {
        ...
        vc.productCode = self.ProductSeriesArray[self.leftSelectIndex];
        vc.productSerialCode = model.ParamValue;
        vc.cartNumber = self.cartNumber;
        [self.navigationController pushViewController:vc animated:YES];
    }
}

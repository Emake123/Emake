...
open class IMUIBaseMessageCell: UICollectionViewCell, IMUIMessageCellProtocol {
  ...
  func setupData(with message: IMUIMessageModelProtocol) {
    let  cornerRadius = self.avatarImage.frame.size.width/2.0
    
    self.avatarImage.layer.cornerRadius = cornerRadius
    self.avatarImage.layer.masksToBounds = true;
    if message.isOutGoing {
        let HeadImage = UserDefaults.standard.string(forKey:"HeadImageURLString")
        if HeadImage != nil{
            ...
            self.avatarImage.image = UIImage.init(named:"kefu_logo")
        }
    
    }else{
        
        self.avatarImage.sd_setImage(with:NSURL.init(string: message.fromUser.Avatar())! as URL , placeholderImage: UIImage.init(named:"guanfangkefu"))
    }
    ...
    if message.isOutGoing {
        ...
        let displayName:NSString = message.fromUser.displayName() as NSString
        if displayName.length == 0 {
            self.nameLabel.isHidden = true
        }else{
            self.nameLabel.isHidden = false
            self.nameLabel.text = (displayName as String)
        }
    }

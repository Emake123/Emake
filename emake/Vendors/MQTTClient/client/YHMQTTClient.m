...
-(void)connectToHost:(NSString *)host Port:(NSInteger)port withUserID:(NSString *)userId{
    ...
    self.mymessageTopic = [NSString stringWithFormat:@"chatroom/%@",userId];

    NSLog(@"messageTopic==%@",self.messageTopic);
    self.nowCount++;
    [self.session connectToHost:MQTT_IP port:MQTT_PORT usingSSL:false connectHandler:^(NSError *error){
        if (!error) {
            ...
            [self subcriceMessageTopic: self.mymessageTopic subcriceAnotherMessageTopic:self.messageAnotherTopic subcriceCMDTopic:self.CMDTopic];
...
- (void)newMessage:(MQTTSession *)session data:(NSData *)data onTopic:(NSString *)topic qos:(MQTTQosLevel)qos retained:(BOOL)retained mid:(unsigned int)mid{
    ...
    if (topicArray.count == 2) {//与客服聊天
        ...
        if ([payload objectForKey:@"MessageType"] != nil) {
            ...
            NSLog(@"收到=消息--qqqqqun-listID--%@",listID);
        }
    }
